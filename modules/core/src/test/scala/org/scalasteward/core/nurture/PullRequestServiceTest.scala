...
import org.scalasteward.core.forge.ForgeType.{Bitbucket, GitHub}
...
class PullRequestServiceTest extends FunSuite {
  ...
  test("createPullRequest() stores info in our local repository on the created PR") {
    ...
    val (boom, (createdPr, prFromRepository)) = (for {
    ...
    } yield (createdPr, prFromRepository)).runSA(mockStateWithExampleRepo).unsafeRunSync()
  ...
  test("listPullRequestsForUpdate() stores info on the PRs it finds in our local repository") {
    val singleUpdate = (dependencyA %> "2.0.0").single
    val updateData = updateDataFor(singleUpdate)
    ...
    val (before, after) = (for {
      _ <- forgeApiAlg.createPullRequest(updateData.repo, newPullRequestDataFor(updateData))
    ...
    } yield (before, after)).runA(mockStateWithExampleRepo).unsafeRunSync()
